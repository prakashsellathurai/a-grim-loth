...
int heap_compare(MinHeap *q, int i, int count, int x) {
  if (count <= 0 || i > q->n)
    return count;

  if (q->q[i] < x) {
    count = heap_compare(q, q->get_young_child(i), count - 1, x);
    count = heap_compare(q, q->get_young_child(i) + 1, count, x);
  }
  return count;
}
...
int main(int argc, char const *argv[]) {
  ...
  std::cout << "is " << 3
            << "th element in the heap is grater than or equal to " << 100
            << " " << ((heap_compare(&pq, 1, 3, 100) != 0) ? "yes" : " NO")
            << std::endl;

  std::cout << "is " << 3
            << "th element in the heap is grater than or equal to " << 1000000
            << " " << ((heap_compare(&pq, 1, 3, 1000000) != 0) ? "yes" : " NO")
            << std::endl;
